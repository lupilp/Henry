...
function problemA() {
  ...
  let pr01 = promisifiedReadFile("poem-two/stanza-01.txt");
  let pr02 = promisifiedReadFile("poem-two/stanza-02.txt");

  Promise.all([pr01, pr02]).then((stanza) => {
    blue(stanza[0]), blue(stanza[1]), console.log("done");
  });
}
...
function problemB() {
  ...
  let poemas = filenames.map((p) =>
    promisifiedReadFile(p).then((stanza) => {
      blue(stanza);
    })
  );

  Promise.all(poemas).then(() => {
    console.log("done");
  });
}
